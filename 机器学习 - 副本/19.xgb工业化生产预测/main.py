...
def xgb_cv(train, test, params, fit_params, feature_names, nfold, seed):
    ...
    for fold_id, (trn_idx, val_idx) in enumerate(kfolder.split(train['收率'])):
        ...
        test_pred['pred'] += xgb_reg.predict(
            xgb_tst, iteration_range=(0, xgb_reg.best_iteration + 1)) / nfold
    # 打印交叉验证损失  # 当前折的均方误差（MSE） # 预测测试集并累加结果
...
if __name__ == '__main__':
    ...
    df_tst_b = pd.read_csv('data/jinnan_round1_testB_20190121.csv', encoding='GB2312')
    print(df_trn.info())
    ...
    df_trn_tst = pd.concat([df_trn, df_tst], ignore_index=False).reset_index(drop=True)
    ...
    for _df in [df_trn_tst, df_trn, df_tst]:
        for _col in cols_timer:
            _df[_col] = _df[_col].map(time_to_min) # 使用 map 函数将每列中的每个值传递
    ...
    # 得到温度相关特征
    df_temperature = df.set_index('样本id') # 设置为索引的列名
